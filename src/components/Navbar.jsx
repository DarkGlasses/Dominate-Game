...
const Navbar = () => {
  ...
  const menuItems = [
    ...
    { name: "Search", link: "/Search", icon: "bi-search" },
    { name: "News", link: "/News", icon: "bi-newspaper" },
    { name: "Playlist", link: "/Playlist", icon: "bi-bookmark" },
    { name: "Reward", link: "/Reward", icon: "bi-gift" },
  ];

  return (
    <>
      {/* Header มือถือ: Burger + Logo */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-md p-4 flex items-center gap-3">
        <button
      ...
      >
        <div className="mt-20 md:mt-0">
          ...
          {/* Menu */}
          <nav className="flex flex-col gap-6 mt-6 px-6 text-white">
            {menuItems.map((item) => {
        ...
        {/* Contact */}
        <div className="bg-red-800 p-4 text-sm">
